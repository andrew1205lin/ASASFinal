...
for kk = 1:numFrames % frame index
    ...
    if sw.visOnly
        ...
        if Lookback == 1 && Tail_rm == 1
            e_n = e_n(length(A) : length(A) + length(ind) - 1); % remove the tail (last p points)
        elseif Lookback == 1 && ~Tail_rm
            e_n = e_n(length(A) : end); 
        elseif Tail_rm == 1
...
folder = "trumpet/";
filename = "excit_p"+ int2str(p)+ "_"+way;
if write_to_disk == 1
    audiowrite(folder+filename+".wav", excitat, fs);
end
